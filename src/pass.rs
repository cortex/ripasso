...
impl PasswordStore {
    ...
    pub fn new(
        password_store_dir: &Option<String>, password_store_signing_key: &Option<String>,
    ...
    pub fn reset(
        &mut self, password_store_dir: &Option<String>, valid_signing_keys: &Option<String>,
    ) -> Result<()> {
        let pass_home = password_dir_raw(password_store_dir);
        if !pass_home.exists() {
            return Err(Error::Generic("failed to locate password directory"));
    ...
    fn verify_gpg_id_file(
        pass_home: &path::PathBuf, signing_keys: &[String],
...
fn push_password_if_match(
    filename: &str, entry_name: &str, commit: &git2::Commit, repo: &git2::Repository,
    dir: &path::PathBuf, passwords: &mut Vec<PasswordEntry>, oid: &git2::Oid,
) -> bool {
...
impl GitLogLine {
    pub fn new(
        message: String, commit_time: DateTime<Local>, signature_status: Option<SignatureStatus>,
    ) -> GitLogLine {
        GitLogLine {
            message,
            commit_time,
...
impl PasswordEntry {
    ...
    pub fn new(
        base: &path::PathBuf, path: &path::PathBuf, update_time: Result<DateTime<Local>>,
        committed_by: Result<String>, signature_status: Result<SignatureStatus>,
    ...
    pub fn load_from_git(
        base: &path::PathBuf, path: &path::PathBuf, repo: &git2::Repository,
    ) -> PasswordEntry {
    ...
    pub fn load_from_filesystem(
        base: &path::PathBuf, path: &path::PathBuf,
...
fn match_with_parent(
    repo: &git2::Repository, commit: &git2::Commit, parent: &git2::Commit,
...
fn commit(
    repo: &git2::Repository, signature: &git2::Signature, message: &str, tree: &git2::Tree,
    parents: &[&git2::Commit],
) -> Result<git2::Oid> {
    if should_sign() {
...
fn add_and_commit_internal(
    repo: &git2::Repository, paths: &[String], message: &str,
) -> Result<git2::Oid> {
    let mut index = repo.index()?;
    for path in paths {
        index.add_path(path::Path::new(path))?;
...
fn cred(
    tried_sshkey: &mut bool, _url: &str, username: Option<&str>, allowed: git2::CredentialType,
