...
fn build_recipient(
    name: String, key_id: String, key_ring_status: KeyRingStatus, trust_level: OwnerTrustLevel,
) -> Recipient {
    Recipient {
        name,
        key_id,
...
impl Recipient {
    ...
    fn write_recipients_file(
        recipients: &[Recipient], recipients_file: &PathBuf, valid_gpg_signing_keys: &[String],
    ) -> Result<()> {
        {
    ...
    pub fn remove_recipient_from_file(
        s: &Recipient, recipient_file: PathBuf, valid_gpg_signing_keys: &[String],
    ) -> Result<()> {
    ...
    pub fn add_recipient_to_file(
        recipient: &Recipient, recipient_file: PathBuf, valid_gpg_signing_keys: &[String],
    ) -> Result<()> {
        let mut recipients: Vec<Recipient> = Recipient::all_recipients(&recipient_file)?;
